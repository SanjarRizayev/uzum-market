    ...
    if (value.length !== 0) {
      ...
      arr.filter((item) => {
        if (item.title.toLowerCase().includes(value)) {
          ...
          bold.push({
            title: item.title.toLowerCase().replace(re, `<b>${value}</b>`),
            id: item.id,
            type: item.type,
          });
      ...
      <header className="flex flex-col gap-[15px] relative">
        {viewport_width > 1024 ? (
            ...
            <div className="flex items-center justify-between h-full px-[25px] lg:w-[1240px] lg:mx-auto lg:px-0">
              <div className="flex items-center gap-[20px]">
                <div className="flex items-center gap-[5px]" onClick={toggle}>
                  ...
                  <p className="text-[14px] text-[#62656a] cursor-pointer">
                    Город:{" "}
                    <span
                      className="text-[#141415] font-[500] underline"
                      id="span"
                    >
                      Самарканд
                    </span>
                  ...
                  <select
                    className="select hidden absolute bottom-[0] z-10"
                ...
                <p className="text-[#141415] font-[500] text-[14px] cursor-pointer duration-[300ms]">
                  Пункты выдачи
                </p>
              </div>
              <p className="text-[#62656a] text-[14px] text-center hidden leading-3 lg:block">
                Доставим ваш заказ бесплатно — всего за 1 день!
              </p>
              <div className="flex items-center gap-[20px]">
                <p className="text-[#62656a] font-[500] text-[14px] cursor-pointer duration-[300ms] hover:text-[#000]">
                  Продавайте на Uzum
                </p>
                <p className="text-[#62656a] font-[500] text-[14px] cursor-pointer duration-[300ms] hover:text-[#000]">
                  Вопрос-ответ
          ...
          <div className="bg-[#f4f5f5] py-[5px]">
            <p className="text-[#62656a] text-[14px] text-center leading-3">
              Доставим ваш заказ бесплатно — всего за 1 день!
            </p>
          </div>
        )}
        <div className="bg-[white] flex flex-col gap-[17px] px-[10px] md:px-[25px] lg:px-[0] lg:w-[1240px] lg:mx-auto">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-[20px] md:gap-[30px]">
              ...
              {viewport_width <= 768 ? (
                ...
                <img
                  ...
                  className="w-[190px] h-[28px] md:w-[215px] md:h-[32px] cursor-pointer"
                  alt=""
                />
              )}
            </div>
            <div className="hidden items-center gap-[10px] w-[52%] md:flex">
              <div
                onClick={() => setOpen(!open)}
                className="cursor-pointer flex items-center px-[16px] justify-between w-[120px] h-[40px] rounded-[4px] duration-[300ms] bg-[#3131c41a] hover:bg-[#3131c427]"
              >
                <svg
                  width="24"
                  height="24"
              ...
              <div className="w-full relative">
                <label className="w-full h-[40px] borderFull rounded-[4px] flex pl-[16px] relative">
                  <Input
                    onKeyUp={(e) => getOptions(e.target.value)}
                    id="inp"
                    variant="unstyled"
                    className="w-full bg-[transparent] outline-none text-[14px] placeholder:text-[#62656a] placeholder:text-[14px] px-[5px]"
                    placeholder="Искать товары и категории"
                  />
                  <div
                    onClick={submit}
                    className="w-[15%] h-full bg-[#76797f0d] flex items-center justify-center"
                  >
                    <img
                      src="../../public/images/search.png"
                      className="w-[19px] h-[19px] cursor-pointer object-contain"
                      alt=""
                    />
                  </div>
                </label>
                <ul
                  id="div"
                  className="w-full h-fit absolute hidden left-0 top-[40px] z-20 bg-[white]"
                ></ul>
              </div>
            </div>
            ...
            <div className="flex items-center">
              <div className="flex items-center gap-[5px] duration-[300ms] py-[9px] rounded-[8px] px-[10px] hover:bg-[#76797f1a] cursor-pointer">
                <div
                  className="w-[22px] h-[22px] cursor-pointer bg-cover"
                  style={{
                    backgroundImage: 'url("../../public/images/chel.png")',
                  }}
                ></div>
                <p className="text-[14px] font-[500] text-center hidden lg:block">
                  Войти
                </p>
              </div>
              <Link
                to={"/liked"}
                className="flex items-center gap-[5px] cursor-pointer duration-[300ms] py-[9px] rounded-[8px] px-[10px] hover:bg-[#76797f1a]"
              >
                <div
                  className="w-[22px] h-[22px] cursor-pointer bg-cover"
                  style={{
                    backgroundImage: 'url("../../public/images/like.png")',
                  }}
                ></div>
                <p className="text-[14px] font-[500] text-center hidden lg:block">
                  Избранное
                ...
                {likedID.length !== 0 ? (
                  <div className="text-[12px] text-[#fff] px-[4px] bg-[#7000ff]">
                    {likedID.length}
                  </div>
                ) : null}
              </Link>
          ...
          <div className="relative md:hidden">
            <label className="flex items-center gap-[7px] bg-[#76797f1a] h-[35px] rounded-[8px] px-[20px]">
              <img
                onClick={submit}
                src="../../public/images/search.png"
                className="w-[19px] h-[19px] invert-[40%] cursor-pointer object-contain"
                alt=""
              />
              <Input
                onKeyUp={(e) => getOptions(e.target.value)}
        ...
        <div className="hidden items-center gap-[25px] h-[39px] md:flex lg:px-[0] lg:w-[1240px] lg:mx-auto">
          <div className="flex items-center gap-[25px] px-[25px] whitespace-nowrap w-[92%] overflow-hidden lg:w-[100%] lg:px-[0]">
            <div className="relative h-full cursor-pointer hover">
              <p className="hover duration-[500ms] text-[#62656a] text-[14px] cursor-pointer">
                Электроника
              </p>
